...
## drive_launch.py
This is the main launch file that launches the ```twist_mixer``` executable and the ```joy_launch.py``` launch file using the following launch arguments:
...
## joy_launch.py
This launch files launches ```joy```, ```teleop_twist_joy``` and ```joy_mode_handler``` nodes and uses the ```joy_config``` launch argument to configure the controller. This node can be launched individually or via ```drive_launch.py``` by setting the ```joy``` launch argument.
